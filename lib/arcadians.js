...
class Arcadians {
  ...
  async getUserNftsWithCustomAddress(customAddress, onNftLoaded) {
    if (typeof window.ethereum !== "undefined") {
      ethereum.on("chainChanged", () => {
        window.location.reload();
      });
      ...
      let currentAddress = customAddress;
      let nftsMetadata;
      try {
        nftsMetadata = await this.getOwnedNftsMetadata(currentAddress);
      } catch {
