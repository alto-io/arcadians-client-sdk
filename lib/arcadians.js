...
class Arcadians {
  constructor(testMode, testAddress, nftsLimit) {
  ...
  createNftsWindow(parsedNftsMetadata, onUserSelect) {
    ...
    //console.log(modal.parentElement.removeChild(modal));
    modal.id = "modal";
    ...
    if (typeof window.ethereum !== "undefined") {
      if (this.nftsWindowOpen === false) {
        ...
        if (parsedNftsMetadata.length > 0) {
          ...
          this.createNftsWindow("You don't own any NFTs.");
        }
      } else {
        console.log("Already showed NFTs window!");
      }
    } else {
      this.createNftsWindow("Please install MetaMask!");
    }
  }
}
